...
function ComingSoonContent() {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center min-h-[calc(100vh-10rem)] bg-white dark:bg-[#0f141c] px-6 transition-colors">
      {/* Mock Notice Banner */}
      <div className="w-full max-w-xl mb-8 bg-amber-50 dark:bg-amber-950/10 border-l-4 border-amber-500 p-4 rounded-r shadow-sm text-xs">
        <div className="flex items-start gap-2.5">
          <AlertTriangle className="w-4 h-4 text-amber-600 dark:text-amber-400 flex-shrink-0 mt-0.5" />
          <div>
            <h4 className="font-bold text-amber-900 dark:text-amber-300 text-xs">Mocked / Non-Functional Section</h4>
            <p className="text-amber-800 dark:text-amber-400 mt-0.5 leading-relaxed text-[11px]">
      ...
      {/* Title */}
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2 text-center">{label}</h1>
      ...
      <p className="text-xs text-gray-400 dark:text-gray-555 text-center max-w-md mb-8">
...
export default function ComingSoonPage() {
  return (
    <Suspense fallback={
      <div className="flex-1 flex items-center justify-center min-h-[calc(100vh-10rem)] bg-white dark:bg-[#0f141c]">
        <Clock className="w-10 h-10 text-gray-300 dark:text-gray-600 animate-pulse" />
      </div>
    }>
      <ComingSoonContent />
    </Suspense>
