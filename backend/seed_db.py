...
async def seed_data_only(session):
...
async def auto_seed_if_empty():
    ...
    print("Checking if database needs auto-seeding...")
    async with AsyncSessionLocal() as session:
        result = await session.execute(select(func.count(HostedZone.id)))
        ...
        if count == 0:
            ...
            print(f"Database already contains {count} hosted zones. Skipping auto-seed.")
